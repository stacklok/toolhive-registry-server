...
SELECT src.source_type AS registry_type,
 ...
 WHERE (sqlc.narg(namespace)::text IS NULL OR p.namespace = sqlc.narg(namespace)::text)
...
SELECT src.source_type AS registry_type,
 ...
 WHERE v.name = sqlc.arg(name)
   ...
   AND (sqlc.narg(namespace)::text IS NULL OR p.namespace = sqlc.narg(namespace)::text)
   AND (
...
-- name: GetPluginVersionBySourceName :one
...
-- name: ListPluginOciPackages :many
SELECT p.id,
       p.plugin_id,
       p.identifier,
       p.digest,
 ...
 WHERE pl.version_id = ANY(sqlc.slice(version_ids)::UUID[]);

-- name: ListPluginGitPackages :many
SELECT p.id,
       p.plugin_id,
       p.url,
       p.ref,
...
INSERT INTO plugin (
    ...
    sqlc.arg(version_id),
    sqlc.arg(namespace),
    COALESCE(sqlc.narg(status)::plugin_status, 'ACTIVE'),
    sqlc.narg(license),
...
-- name: InsertPluginOciPackage :exec
...
-- name: InsertPluginGitPackage :exec
...
-- name: DeletePluginsByRegistry :exec
...
-- name: InsertPluginVersionForSync :one
INSERT INTO plugin (
    version_id,
    namespace,
    status,
...
-- name: UpsertPluginVersionForSync :one
INSERT INTO plugin (
    version_id,
    namespace,
    status,
...
-- name: DeletePluginOciPackagesByPluginId :exec
DELETE FROM plugin_oci_package WHERE plugin_id = sqlc.arg(plugin_id);

-- name: DeletePluginGitPackagesByPluginId :exec
DELETE FROM plugin_git_package WHERE plugin_id = sqlc.arg(plugin_id);
