...
func TestExampleConfigurations(t *testing.T) {
	...
	for _, filename := range exampleFiles {
		t.Run(filename, func(t *testing.T) {
			...
			assert.Contains(t, []string{SourceTypeConfigMap, SourceTypeGit, SourceTypeAPI, SourceTypeFile},
...
func TestExampleConfigurationDetails(t *testing.T) {
	...
	tests := []struct {
		...
		{
			filename:   "config-configmap.yaml",
			sourceType: SourceTypeConfigMap,
			validateFunc: func(t *testing.T, cfg *Config) {
				require.NotNil(t, cfg.Source.ConfigMap, "ConfigMap config should be set")
				assert.NotEmpty(t, cfg.Source.ConfigMap.Name, "ConfigMap name should be set")
				// Namespace may be empty (defaults to current namespace)
			},
		},
		{
