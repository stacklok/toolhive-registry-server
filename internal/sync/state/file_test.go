...
func TestFileStateService_UpdateStatusAtomically(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		registryName    string
		cachedStatuses  map[string]*status.SyncStatus
		testAndUpdateFn func(*status.SyncStatus) bool
		setupMocks      func(*statusmocks.MockStatusPersistence)
		wantUpdated     bool
		wantErr         bool
		errMessage      string
		...
		{
			name:         "updates when test function returns true",
			registryName: "registry1",
			cachedStatuses: map[string]*status.SyncStatus{
				"registry1": {
					Phase:       status.SyncPhaseFailed,
			...
			testAndUpdateFn: func(s *status.SyncStatus) bool {
				if s.Phase == status.SyncPhaseFailed {
					s.Phase = status.SyncPhaseComplete
					s.Message = "Fixed"
					return true
				}
				return false
			...
			setupMocks: func(m *statusmocks.MockStatusPersistence) {
				m.EXPECT().SaveStatus(gomock.Any(), "registry1", gomock.Any()).DoAndReturn(
					func(_ context.Context, _ string, s *status.SyncStatus) error {
						assert.Equal(t, status.SyncPhaseComplete, s.Phase)
						assert.Equal(t, "Fixed", s.Message)
						return nil
					})
			},
			wantUpdated: true,
		...
		{
			name:         "doesn't update when test function returns false",
			registryName: "registry1",
			cachedStatuses: map[string]*status.SyncStatus{
				"registry1": {
					Phase:       status.SyncPhaseComplete,
			...
			testAndUpdateFn: func(s *status.SyncStatus) bool {
				...
				return s.Phase != status.SyncPhaseComplete
			},
			setupMocks:  func(_ *statusmocks.MockStatusPersistence) {},
			wantUpdated: false,
			wantErr:     false,
		...
		{
			name:            "returns error for non-existent registry",
			registryName:    "non-existent",
			cachedStatuses:  map[string]*status.SyncStatus{},
			testAndUpdateFn: func(_ *status.SyncStatus) bool { return true },
			setupMocks:      func(_ *statusmocks.MockStatusPersistence) {},
			wantUpdated:     false,
			wantErr:         true,
			errMessage:      "sync status for registry non-existent not found",
		},
		{
			name:         "returns error for nil status",
			registryName: "registry1",
			cachedStatuses: map[string]*status.SyncStatus{
				"registry1": nil,
			},
			...
			wantUpdated:     false,
			wantErr:         true,
			errMessage:      "sync status for registry registry1 not found",
		},
		{
			name:         "handles save error",
			registryName: "registry1",
		...
		},
		{
			...
			testAndUpdateFn: func(s *status.SyncStatus) bool {
				// Reset attempt count if > 3
				if s.AttemptCount > 3 {
					s.AttemptCount = 0
					s.Message = "Reset after max attempts"
					return true
				}
				// Increment attempt count if failed
				if s.Phase == status.SyncPhaseFailed && s.AttemptCount <= 3 {
					s.AttemptCount++
					s.Message = "Retry attempt"
					return true
				}
				return false
			},
			setupMocks: func(m *statusmocks.MockStatusPersistence) {
				m.EXPECT().SaveStatus(gomock.Any(), "registry1", gomock.Any()).DoAndReturn(
					func(_ context.Context, _ string, s *status.SyncStatus) error {
						assert.Equal(t, 4, s.AttemptCount)
						assert.Equal(t, "Retry attempt", s.Message)
						return nil
					})
			},
			wantUpdated: true,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			updated, err := service.UpdateStatusAtomically(ctx, tt.registryName, tt.testAndUpdateFn)
...
func TestFileStateService_ThreadSafety(t *testing.T) {
	...
	// Set up mock expectations for concurrent updates
	mockPersistence.EXPECT().SaveStatus(gomock.Any(), gomock.Any(), gomock.Any()).
		Return(nil).AnyTimes()
	...
	for i := 0; i < numGoroutines; i++ {
		...
		go func(i int) {
			...
			case 1:
				// Get specific status
				status, err := service.GetSyncStatus(ctx, "registry1")
				assert.NoError(t, err)
				_ = status
			case 2:
				// Update status
				...
				err := service.UpdateSyncStatus(ctx, "registry1", newStatus)
				assert.NoError(t, err)
			case 3:
				// Test and update
				_, err := service.UpdateStatusAtomically(ctx, "registry1", func(s *status.SyncStatus) bool {
					s.ServerCount++
					return true
				})
	...
	// Verify data integrity - should still have entries in cache
	assert.NotEmpty(t, service.cachedStatuses)
}
...
func TestFileStateService_NilHandling(t *testing.T) {
	t.Parallel()
	...
	// Set up cache with nil values
	service.cachedStatuses = map[string]*status.SyncStatus{
		"registry1": {Phase: status.SyncPhaseComplete},
		"registry2": nil,
		"registry3": {Phase: status.SyncPhaseFailed},
	}

	ctx := context.Background()

	t.Run("ListSyncStatuses filters nil values", func(t *testing.T) {
		t.Parallel()

		statuses, err := service.ListSyncStatuses(ctx)
		require.NoError(t, err)

		// Should only have non-nil entries
		assert.Len(t, statuses, 2)
		assert.Contains(t, statuses, "registry1")
		assert.Contains(t, statuses, "registry3")
		assert.NotContains(t, statuses, "registry2")
	})

	t.Run("GetSyncStatus returns nil for nil cached value", func(t *testing.T) {
		t.Parallel()

		status, err := service.GetSyncStatus(ctx, "registry2")
		require.NoError(t, err)
		assert.Nil(t, status)
	})

	t.Run("UpdateStatusAtomically errors on nil cached value", func(t *testing.T) {
		t.Parallel()

		updated, err := service.UpdateStatusAtomically(ctx, "registry2", func(_ *status.SyncStatus) bool {
			return true
		})
		require.Error(t, err)
		assert.False(t, updated)
		assert.Contains(t, err.Error(), "sync status for registry registry2 not found")
	})
}
